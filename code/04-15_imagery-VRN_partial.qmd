...
```{r pre_ndre}
pre_ndre <- read_stars("../data/ndre_pre_v12.tif") %>% 
  st_downsample(n = 3) %>% #making it coarser three times, where delta will be increased three times
  rename(ndre = ndre_pre_v12.tif)
pre_ndre
```

```{r plots}
plots <- read_sf("../data/plots.geojson") %>% 
  janitor::clean_names()
...
pre_ndre_map <- ggplot()+
  geom_stars(data = pre_ndre)+
  scale_fill_viridis_c()+
  coord_equal()+ #set the coordinates to have equal spacing so that it won't compress the plot 
  theme_map()+
...
grid_vrn <- plots %>% 
  # Making grid
  st_make_grid(cellsize = c(6, 1200)) %>% 
  as("Spatial") %>%
  #Rotating it to match the field angle
  #elide(rotate = -2.2, # trying to match the angle of field, here i couldn't install the maptools package so its commented
  # center = plots %>%
  # st_union() %>%
  # st_centroid() %>%
  # st_coordinates()) %>%
  st_as_sf() %>% 
  st_set_crs(value = 32614) %>% 
...
```{r vrn_ref}
vrn_ref <- quantile(pre_ndre$ndre, probs = .95,
                    na.rm = T
                    )
...
ggplot()+
  geom_stars(data = grid_ndre)+
  geom_sf(data = grid_vrn, fill = NA)+
  scale_fill_viridis_c()+
  theme_map()+
  theme(legend.position = "bottom")
```
...
```{r grid_ndre_av}
grid_ndre_av <- grid_ndre %>% 
  st_extract(grid_vrn,              #st_extract in R is like zonal statistics in arcgis or qgis
              FUN = mean,
              na.rm = T
              ) %>% 
  st_as_sf()
...
si <- grid_ndre_av %>% 
  # Setting reference
  mutate(ndre_ref = vrn_ref) %>% 
  # Calculating SI 
  mutate(si = ndre/ndre_ref)
...
vrn <- si %>% 
  mutate(nopt = 229,
         ...
         ncred = 20) %>% 
  # All in lbs/ac
  mutate(vrn_lbsac = (nopt - npre - ncred )*(sqrt(((1-si)/0.3))))

...
```{r yieldic17}
yieldic17 <- read_sf("../../07_yield_monitor/data/yield17_interpolated.geojson") %>% 
  dplyr::select(yield17_lbac = ipyield_lbac)
...
```{r yieldic19}
yieldic19 <- read_sf("../../07_yield_monitor/data/yield19_interpolated.geojson") %>%
...
```{r yieldic20}
yieldic20 <- read_sf("../../07_yield_monitor/data/yield20_interpolated.geojson") %>%
...
```{r zone_st}
zone_st <- read_sf("../../11_management_zones/data/zone_s.geojson") %>% 
  dplyr::select(zone)
...
vrn <- zone_yield %>% 
       pivot_longer(cols = yield17_lbac:yield20_lbac) %>% 
  group_by(zone) %>% 
  summarise(yieldpotential = quantile(value, probs = 0.9)) %>% 
  ...
  mutate(totalnrate_adj_lbsac = totalnrate_lbsac*1.25) %>% # we doing this because production guide says to increase rate by 25% if cotton is following cotton. If there was soybean or peanuts, you reduce the rate. 
  # Calculate pre-plant rate
  mutate(pp_totalnrate_adj_lbsac = totalnrate_adj_lbsac*0.33) %>% #preplant we apply the same rate, here its 55 lb/ac
  # Calculate ~2/3 as portion to be applied in-season (Assuming a single pre-plant rate was applied to both zones)
  mutate(is_totalnrate_adj_lbsac = totalnrate_adj_lbsac - 55) %>%
...
```
From UGFertex, for high yield potential = 135 lbs/ac, low potential = 115 lbs/ac
...
```
 ...
 The farmer need to order 4235 gallons of UAN for the field for in-season application. 
...
- Calculated VRN in both lbs N/ac and gallons UAN28/ac  
